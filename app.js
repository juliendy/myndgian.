...
function updateStats() {
    ...
    const prioritisedItemsText = document.getElementById(
        "prioritisedItemsText"
    );
...
const settingsInnerContainer = document.querySelector(
    ".settings-inner-container"
);
const settingsModalBox = document.querySelector(
...
function closeSettingsInnerModal() {
    ...
    settingsModalBox.classList.remove(
        "edit-username-active",
        "edit-list-name-active",
        "reset-list-active"
    );
...
function openEditUsername() {
    ...
    smallModalHeader.innerHTML =
        'Change Your <span class="highlight">Username</span>';
...
function openEditListName() {
    openSettingsInnerModal();
    smallModalHeader.innerHTML =
        'Change Your <span class="highlight">List Name</span>';
...
function openResetList() {
    openSettingsInnerModal();
    smallModalHeader.innerHTML =
        '<span class="highlight-negative">Reset</span> Your List?';
    smallModalP1.innerHTML =
        "You will be unable to recover your data once your list has been reset.";
...
smallModalBtnMain.addEventListener("click", () => {
    if (settingsModalBox.classList.contains("edit-username-active")) {
        ...
        const recentActivityItemContainer = targetItem.querySelector(
            ".content-text--to-do"
        );
    ...
    if (pressed === "Enter") {
        if (settingsInnerContainer.classList.contains("active")) {
            if (settingsModalBox.classList.contains("edit-username-active")) {
                ...
                settingsModalBox.classList.contains("edit-list-name-active")
            ) {
                if (smallModalInput.value != "") {
                    // Change List Name
                    listName = smallModalInput.value;
                ...
                settingsModalBox.classList.contains("reset-list-active")
            ) {
                // Reset List
                resetList();
            } else if (
                settingsModalBox.classList.contains("add-to-do-active")
            ) {
                // Add Item to List (Small Screen)
                if (smallModalInput.value !== "") {
                    addToDo();
                ...
                settingsModalBox.classList.contains("edit-item-active")
            ) {
                if (smallModalInput.value != "") {
                    // Edit Item
                    itemToEdit.textContent = smallModalInput.value;
                ...
                settingsModalBox.classList.contains("delete-item-active")
            ) {
                // Delete Item
                if (targetItem.classList.contains("item-priority")) {
                    if (targetItem.classList.contains("item-complete")) {
...
function openAddToDoModal() {
    openSettingsInnerModal();
    smallModalHeader.innerHTML =
        '<span class="highlight">Add</span> a To-Do Item';
...
function completeToDoItem(item, icon) {
    const recentActivityItemContainer = item.querySelector(
        ".content-text--to-do"
    );
    recentActivityItem = recentActivityItemContainer.textContent;
...
function openEditItemModal() {
    openSettingsInnerModal();
    smallModalHeader.innerHTML =
        '<span class="highlight">Edit</span> Your To-Do Item';
...
function openDeleteItemModal() {
    openSettingsInnerModal();
    smallModalHeader.innerHTML =
        '<span class="highlight-negative">Delete</span> Your To-Do Item?';
    smallModalP1.innerHTML =
        "You will be unable to recover your data once it has been deleted.";
    smallModalP2.innerHTML = "Are you sure?";
    smallModalInput.style.display = "none";
    smallModalBtnMain.classList.add("btn-negative");
    smallModalBtnMain.classList.remove("btn-primary");
...
function checkLevel(level) {
    const lvl1Quotes = [
        ...
        `Add tasks to your list and complete them to earn extra points!`,
        `You can earn 10 points when you complete a task, or 20 points when you complete a prioritised task!`,
        `You can get 5 points for simply creating a to-do item! What are you waiting for?`,
    ];
    const lvl2Quotes = [...lvl1Quotes, `You're doing great! Keep going!`];
    const lvl3Quotes = [
        ...lvl2Quotes,
        `This is some productivity! There's no stopping you!`,
    ];
    const lvl4Quotes = [
        ...lvl3Quotes,
        `Level ${level}?! You're a task machine!`,
    ];

    switch (level) {
        case 1:
            quoteText.textContent =
                lvl1Quotes[Math.floor(Math.random() * lvl1Quotes.length)];
            break;
        case 2:
            quoteText.textContent =
                lvl2Quotes[Math.floor(Math.random() * lvl2Quotes.length)];
            break;
        case 3:
            quoteText.textContent =
                lvl3Quotes[Math.floor(Math.random() * lvl3Quotes.length)];
            break;
        case 4:
            quoteText.textContent =
                lvl4Quotes[Math.floor(Math.random() * lvl4Quotes.length)];
            break;
        default:
